...
def detect_num_frames(images_path: Path, mode="bin"):
    if mode == "bin":
        return len(list(read_images_binary(images_path).values()))
    elif mode == "txt":
        return len(list(read_images_text(images_path).values()))
    else:
