...
def eval_load_checkpoint(config: TrainerConfig, pipeline: Pipeline) -> Tuple[Path, int]:
    ...
    if "filtered_train_indices" in loaded_state and loaded_state["filtered_train_indices"] is not None:
        pipeline.datamanager.filtered_train_indices = loaded_state["filtered_train_indices"]
        num_filtered = len(loaded_state["filtered_train_indices"])
        CONSOLE.print(f"Loaded {num_filtered} filtered frame indices from checkpoint")
