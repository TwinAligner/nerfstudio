...
class CacheDataloader(DataLoader):
    ...
    def _get_batch_list(self):
        ...
        # Filter out indices if they are marked as filtered
        if hasattr(self, 'filtered_indices') and self.filtered_indices is not None:
            filtered_set = set(self.filtered_indices.tolist())
            all_indices = [idx for idx in all_indices if idx not in filtered_set]
        
        # Sample from available indices
        num_to_sample = min(self.num_images_to_sample_from, len(all_indices))
        indices = random.sample(all_indices, k=num_to_sample)
