...
class Nerfstudio(DataParser):
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        for frame in meta["frames"]:
            ...
            depth_filepath = Path(
                str(frame["file_path"]).replace("images", "depths")
            ).with_suffix(".png")
            ...
            if self.config.reconstruct_part_id is not None:
                depth_fname = Path(str(depth_fname).replace("depths", f"depths_part_{self.config.reconstruct_part_id}"))
            if depth_fname.exists():
                depth_filenames.append(depth_fname)

            if depth_fname.exists():
                mask_filepath = Path(
                    str(frame["file_path"]).replace("images", "depth_masks")
                )
                mask_fname = self._get_fname(
                    mask_filepath,
                    data_dir,
                    downsample_folder_prefix="depth_masks_",
                )
            else:
                mask_filepath = Path(
            ...
            if self.config.reconstruct_part_id is not None:
                mask_fname = Path(str(mask_fname).replace("masks", f"masks_part_{self.config.reconstruct_part_id}"))
        ...
        if self.config.resume_sdfstudio_dir is not None and split == "train" and not self.config.dont_resume_c2w:
            new_c2w_path = self.config.resume_sdfstudio_dir / "new_c2w.npz"
            new_c2w = np.load(new_c2w_path)["c2w"]
            cameras.camera_to_worlds = torch.from_numpy(new_c2w).type(cameras.camera_to_worlds.dtype).to(cameras.camera_to_worlds.device)
            ...
            if ply_file_path:
                ...
                if str(ply_file_path).endswith("object.urdf"):
                    ...
                    for link in urdf_robot.links:
                        ...
                        points3D_xyz = torch.cat([points3D_xyz, sparse_points["points3D_xyz"]], dim=0)
                        points3D_rgb = torch.cat([points3D_rgb, sparse_points["points3D_rgb"]], dim=0)
                        points3D_partidx = torch.cat([points3D_partidx, torch.full((sparse_points["points3D_xyz"].size(0), ), part_idx, dtype=torch.int)], dim=0)
                    sparse_points = {
