...
def render_trajectory(
    ...
    # )
    # with progress:
    for camera_idx in range(cameras.size):#progress.track(range(cameras.size), description=""):
        if isinstance(pipeline.model, SplatfactoModel):
            with torch.no_grad():
                outputs = pipeline.model.get_outputs_for_camera(cameras[camera_idx:camera_idx+1])
        else:
...
def collect_camera_poses_for_dataset(dataset: Optional[InputDataset], pipeline: Optional[VanillaPipeline] = None, optimized: bool = False, world_frame: bool = False, filtered_indices: Optional[torch.Tensor] = None) -> List[Dict[str, Any]]:
    """Collects rescaled, translated and optimised camera poses for a dataset.

    Args:
        dataset: Dataset to collect camera poses for.
        pipeline: Pipeline used to access camera optimizer and dataparser transforms.
        optimized: If True, apply model's camera optimizer before exporting.
        world_frame: If True, transform poses back to the original world coordinate system.
        filtered_indices: Indices of frames to exclude from export.
    ...
    # Start with cameras' current c2w (in normalized training space)
    c2w = cameras.camera_to_worlds
    if optimized:
        c2w = pipeline.model.camera_optimizer.apply_to_camera(cameras).cpu()
    if world_frame:
        c2w = pipeline.datamanager.train_dataparser_outputs.transform_poses_to_original_space(c2w)
    
    # Create a set of filtered indices for fast lookup
    filtered_set = set()
    if filtered_indices is not None:
        filtered_set = set(filtered_indices.tolist() if torch.is_tensor(filtered_indices) else filtered_indices)
    
    frames: List[Dict[str, Any]] = []

    for idx in range(len(cameras)):
        # Skip filtered frames
        if idx in filtered_set:
            continue
            
        image_filename = image_filenames[idx]
        transform = c2w[idx].tolist()
        frames.append({"file_path": str(image_filename), "transform": transform})

    return frames


def collect_camera_poses(pipeline: VanillaPipeline, optimized: bool = False, world_frame: bool = False) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    # Get filtered indices if they exist
    filtered_indices = None
    if hasattr(pipeline.datamanager, 'filtered_train_indices'):
        filtered_indices = pipeline.datamanager.filtered_train_indices

    train_frames = collect_camera_poses_for_dataset(train_dataset, pipeline=pipeline, optimized=optimized, world_frame=world_frame, filtered_indices=filtered_indices)
    eval_frames = collect_camera_poses_for_dataset(eval_dataset, pipeline=pipeline, optimized=optimized, world_frame=world_frame)
