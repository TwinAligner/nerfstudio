...
def export_tsdf_mesh(
    ...
    c2w = pipeline.model.camera_optimizer.apply_to_camera(cameras).to(device)
    # make c2w homogeneous
    c2w = torch.cat([c2w, torch.zeros(c2w.shape[0], 1, 4, device=device)], dim=1)
    c2w[:, 3, 3] = 1
    # Hongwei: NeRFStudio --> Open3D axes
    c2w[:, :3, 1:3] *= -1
    ...
    extrinsic = c2w.cpu().numpy()
    for i in tqdm(range(0, len(c2w), 1)):
        color = o3d.geometry.Image((color_images[i]*255).astype(np.uint8))
        depth = pipeline.datamanager.train_dataset.get_data(i)['depth_image'].cpu().numpy()
        depth = o3d.geometry.Image((depth))
        rgbd = o3d.geometry.RGBDImage.create_from_color_and_depth(
            color, depth, depth_scale=1.0, depth_trunc=1, convert_rgb_to_intensity=False)
        intrinsic = o3d.camera.PinholeCameraIntrinsic(cameras[i].width.cpu().numpy()[0], 
                                                      cameras[i].height.cpu().numpy()[0], 
                                                      K[i, 0, 0], 
                                                      K[i, 1, 1], 
                                                      K[i, 0, 2], 
                                                      K[i, 1, 2])
        volume.integrate(
            rgbd,
            intrinsic,
            np.linalg.inv(extrinsic[i]),
        )
        # pcd = o3d.geometry.PointCloud.create_from_rgbd_image(
        #     rgbd, intrinsic=intrinsic, extrinsic=np.linalg.inv(extrinsic.cpu().numpy())
        # )
        # o3d.io.write_point_cloud(str(pcd_dir / "frame_{:05d}.ply".format(i)), pcd)
    ...
    o3d.io.write_triangle_mesh(str(output_dir / "tsdf_fusion.ply"), mesh, 
