...
descriptions = {
    ...
    "splatfacto": "3DGS, with possible mask supervision",
    "splatfacto-depth": "3DGS with depth supervision, for large rigid objects, with possible mask supervision",
    "twopart-blend": "Two-part GS blending with per-point weights and per-frame SE(3) interpolation.",
    "splatfacto-art": "Articulated GS with explicit joint axis and angle optimization."
}
...
method_configs["splatfacto-depth"] = TrainerConfig(
    method_name="splatfacto-depth",
    steps_per_eval_image=100,
    steps_per_eval_batch=0,
    steps_per_save=2000,
    steps_per_eval_all_images=1000,
    ...
    optimizers={
        ...
        "camera_opt": {
            "optimizer": AdamOptimizerConfig(lr=1e-4, eps=1e-15),
            "scheduler": ExponentialDecaySchedulerConfig(
                lr_final=5e-5, max_steps=30000, warmup_steps=1000, lr_pre_warmup=0
            ),
        },
    },
    viewer=ViewerConfig(num_rays_per_chunk=1 << 15),
...
TWOPART_BLEND_GS_LR_SCALE = 0.2
method_configs["twopart-blend"] = TrainerConfig(
    method_name="twopart-blend",
    steps_per_eval_image=100,
    steps_per_eval_batch=0,
    steps_per_save=2000,
    steps_per_eval_all_images=1000,
    ...
    pipeline=VanillaPipelineConfig(
        datamanager=FullImageDatamanagerConfig(
            _target=FullImageDatamanager[DepthDataset],
            dataparser=TwoPartBlendDataParserConfig(),
            cache_images_type="uint8",
        ),
        model=TwoPartBlendModelConfig(
            camera_optimizer=CameraOptimizerConfig(mode="SO3xR3"),
            background_color="white",
        ),
    ),
    optimizers={
        "means": {
            "optimizer": AdamOptimizerConfig(lr=1.6e-4 * TWOPART_BLEND_GS_LR_SCALE, eps=1e-15),
            "scheduler": ExponentialDecaySchedulerConfig(
                lr_final=1.6e-6,
                max_steps=30000,
                warmup_steps=0,
        ...
        "features_dc": {
            "optimizer": AdamOptimizerConfig(lr=0.0025 / 20 * TWOPART_BLEND_GS_LR_SCALE, eps=1e-15),
            "scheduler": ExponentialDecaySchedulerConfig(
                lr_final=0.0025 / 20 * TWOPART_BLEND_GS_LR_SCALE,
                max_steps=30000,
                warmup_steps=0,
                lr_pre_warmup=0,
                ramp="none",
        ...
        "opacities": {
            "optimizer": AdamOptimizerConfig(lr=0.005 * TWOPART_BLEND_GS_LR_SCALE, eps=1e-15),
            "scheduler": ExponentialDecaySchedulerConfig(
                lr_final=0.005 * TWOPART_BLEND_GS_LR_SCALE,
                max_steps=30000,
                warmup_steps=0,
                lr_pre_warmup=0,
                ramp="none",
        ...
        "scales": {
            "optimizer": AdamOptimizerConfig(lr=0.0005 * TWOPART_BLEND_GS_LR_SCALE, eps=1e-15),
            "scheduler": ExponentialDecaySchedulerConfig(
                lr_final=0.0005 * TWOPART_BLEND_GS_LR_SCALE,
                max_steps=30000,
                warmup_steps=0,
                lr_pre_warmup=0,
                ramp="none",
        ...
        "quats": {"optimizer": AdamOptimizerConfig(lr=0.001 * TWOPART_BLEND_GS_LR_SCALE, eps=1e-15), "scheduler": ExponentialDecaySchedulerConfig(
                lr_final=0.001 * TWOPART_BLEND_GS_LR_SCALE,
                max_steps=30000,
                warmup_steps=0,
                lr_pre_warmup=0,
                ramp="none",
        ...
        "part_weights": {"optimizer": AdamOptimizerConfig(lr=1e-1, eps=1e-15), "scheduler": ExponentialDecaySchedulerConfig(
                lr_final=1e-1,
                max_steps=30000,
                warmup_steps=5000,
                lr_pre_warmup=0,
...
method_configs["splatfacto-art"] = TrainerConfig(
    ...
    steps_per_save=2000,
    steps_per_eval_all_images=1000,
    max_num_iterations=SPLATFACTO_ITERATIONS,
    mixed_precision=False,
    pipeline=VanillaPipelineConfig(
        datamanager=FullImageDatamanagerConfig(
            _target=FullImageDatamanager[DepthDataset],
            dataparser=SplatfactoArtDataParserConfig(),
            cache_images_type="uint8",
        ),
        model=SplatfactoArtModelConfig(
            camera_optimizer=CameraOptimizerConfig(mode="SO3xR3"),
            background_color="white",
            warmup_length=100,  # Short warmup since we start from pre-trained GS
            refine_every=100,
            stop_split_at=30000,  # Early stop for refinement
            trainable_weights=True,
            optimize_joint_axis=True,
            optimize_joint_origin=True,
            optimize_joint_params=True,
            axis_adjustment_weight=0,
            origin_adjustment_weight=0,
            param_smoothness_weight=0,
            use_second_order_smoothness=True,
        ),
    ),
    optimizers={
        # Gaussian parameters - smaller learning rates since pre-trained
        "means": {
            "optimizer": AdamOptimizerConfig(lr=1.6e-4 * SPLATFACTO_ART_GS_LR_SCALE, eps=1e-15),  # 10x smaller
            "scheduler": ExponentialDecaySchedulerConfig(
                lr_final=1.6e-4 * SPLATFACTO_ART_GS_LR_SCALE,
                max_steps=SPLATFACTO_ITERATIONS,
                warmup_steps=5000,
                lr_pre_warmup=0,
                ramp="none",
            ),
        ...
        "features_dc": {
            "optimizer": AdamOptimizerConfig(lr=0.0025 / 20 * SPLATFACTO_ART_GS_LR_SCALE, eps=1e-15),
            "scheduler": ExponentialDecaySchedulerConfig(
                lr_final=0.0025 / 20 * SPLATFACTO_ART_GS_LR_SCALE,
                max_steps=SPLATFACTO_ITERATIONS,
                warmup_steps=5000,
                lr_pre_warmup=0,
                ramp="none",
        ...
        "opacities": {
            "optimizer": AdamOptimizerConfig(lr=0.005 * SPLATFACTO_ART_GS_LR_SCALE, eps=1e-15),  # 10x smaller
            "scheduler": ExponentialDecaySchedulerConfig(
                lr_final=0.005 * SPLATFACTO_ART_GS_LR_SCALE,
                max_steps=SPLATFACTO_ITERATIONS,
                warmup_steps=5000,
                lr_pre_warmup=0,
                ramp="none",
        ...
        "scales": {
            "optimizer": AdamOptimizerConfig(lr=0.0005 / 2 * SPLATFACTO_ART_GS_LR_SCALE, eps=1e-15),  # 10x smaller
            "scheduler": ExponentialDecaySchedulerConfig(
                lr_final=0.0005 / 2 * SPLATFACTO_ART_GS_LR_SCALE / 100,
                max_steps=SPLATFACTO_ITERATIONS,
                warmup_steps=5000,
                lr_pre_warmup=0,
                ramp="none",
        ...
        "quats": {
            "optimizer": AdamOptimizerConfig(lr=0.001 * SPLATFACTO_ART_GS_LR_SCALE, eps=1e-15),  # 10x smaller
            "scheduler": ExponentialDecaySchedulerConfig(
                lr_final=0.001 * SPLATFACTO_ART_GS_LR_SCALE,
                max_steps=SPLATFACTO_ITERATIONS,
                warmup_steps=5000,
                lr_pre_warmup=0,
                ramp="none",
        ...
        # Trainable per-point part weights
        "part_weights": {
            "optimizer": AdamOptimizerConfig(lr=1e-3, eps=1e-15),
            "scheduler": ExponentialDecaySchedulerConfig(
                lr_final=1e-3,
                max_steps=SPLATFACTO_ITERATIONS/2,
                warmup_steps=5000,
                lr_pre_warmup=0,
                ramp="none",
            ),
        },
        # Joint parameters - main optimization focus
        "joint_axis_adjustment": {
            "optimizer": AdamOptimizerConfig(lr=5e-4 * SPLATFACTO_ART_OPT_LR_SCALE, eps=1e-15, betas=(0.5, 0.999)),
            "scheduler": ExponentialDecaySchedulerConfig(
                lr_final=1e-5 * SPLATFACTO_ART_OPT_LR_SCALE, max_steps=SPLATFACTO_ITERATIONS, warmup_steps=0, lr_pre_warmup=0
            ),
        },
        "joint_origin": {
            "optimizer": AdamOptimizerConfig(lr=1e-2 * SPLATFACTO_ART_OPT_LR_SCALE, eps=1e-15, betas=(0.5, 0.999)),
            "scheduler": ExponentialDecaySchedulerConfig(
                lr_final=1e-4 * SPLATFACTO_ART_OPT_LR_SCALE, max_steps=SPLATFACTO_ITERATIONS, warmup_steps=0, lr_pre_warmup=0
            ),
        },
        "joint_params": {
            "optimizer": AdamOptimizerConfig(lr=1e-3 * SPLATFACTO_ART_OPT_LR_SCALE, eps=1e-15, betas=(0.5, 0.999)),
            "scheduler": ExponentialDecaySchedulerConfig(
                lr_final=1e-5 * SPLATFACTO_ART_OPT_LR_SCALE, max_steps=SPLATFACTO_ITERATIONS, warmup_steps=0, lr_pre_warmup=0
            ),
        },
        "rotation_offset": {
            "optimizer": AdamOptimizerConfig(lr=1e-4 * SPLATFACTO_ART_OPT_LR_SCALE, eps=1e-15, betas=(0.5, 0.999)),
            "scheduler": ExponentialDecaySchedulerConfig(
                lr_final=5e-6 * SPLATFACTO_ART_OPT_LR_SCALE, max_steps=SPLATFACTO_ITERATIONS, warmup_steps=0, lr_pre_warmup=0
            ),
        },
        # Camera pose optimization, enabled after warmup via scheduler
        "camera_opt": {
            "optimizer": AdamOptimizerConfig(lr=1e-4, eps=1e-15, betas=(0.5, 0.999)),
            "scheduler": ExponentialDecaySchedulerConfig(
                lr_final=5e-7, max_steps=SPLATFACTO_ITERATIONS, warmup_steps=10000, lr_pre_warmup=0
            ),
        },
    },
    viewer=ViewerConfig(num_rays_per_chunk=1 << 15),
