...
class CameraOptimizer(nn.Module):
    ...
    def apply_to_camera(self, camera: Cameras) -> torch.Tensor:
        ...
        if camera.camera_to_worlds.shape[0] == 0:
            return camera.camera_to_worlds
        ...
        if isinstance(camera_idx, torch.Tensor) and len(camera_idx.shape) == 2:
            camera_idx = camera_idx[0]
            c2w = camera.camera_to_worlds.detach().to("cuda")
            adj = self(camera_idx)  # type: ignore
            adj = torch.cat([adj, adj.new_tensor([0, 0, 0, 1]).view(1, 1, 4).repeat(adj.shape[0], 1, 1)], dim=1)
            # import ipdb; ipdb.set_trace()
            return torch.bmm(c2w, adj)
        else:
