...
def auto_orient_and_center_poses(
    poses: Float[Tensor, "*num_poses 4 4"], method: Literal["pca", "up", "none"] = "up", center_poses: bool = True
) -> Tuple[Float[Tensor, "*num_poses 3 4"], Float[Tensor, "3 4"]]:
    ...
    cal_poses = poses[:, :3, :]
    cams_ori = cal_poses[...,3]
    cams_dir = cal_poses[:,:3,:3] @ torch.as_tensor([0.,0.,-1.])
    ...
    mean_translation = (torch.stack([cams_dir, cams_dir.roll(1,0)], dim=-1) * t[:,None,:] + torch.stack([cams_ori, cams_ori.roll(1,0)], dim=-1)).mean((0,2))
