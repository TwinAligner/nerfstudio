...
def parse_object(
    ...
    obj_props: List[Tuple[str, Any]] = []
    for k in dir(obj):
        if isinstance(getattr(type(obj), k, None), cached_property):
            continue
        try:
            v = getattr(obj, k)
        except AttributeError:
            # some objects advertise attributes in dir() that are not actually present
            # accessing them can raise AttributeError; skip safely
            continue
        except Exception:
            # defensively skip any attribute that raises during access
            continue
        obj_props.append((k, v))
