...
def test_equall_nerfacc():
    """
    test that the output of intersect_aabb is close to nerfacc_nerfstudio.intersect_aabb
    :return:
    """
    check_nerf_acc = strtobool(os.environ.get("INTERSECT_WITH_nerfacc_nerfstudio", "FALSE"))
    if check_nerf_acc:
        nerfacc_nerfstudio = importlib.import_module("nerfacc_nerfstudio")
        random.seed(8128)
        ...
        # time3 = time.time()
        t_min_nerfacc_nerfstudio, t_max_nerfacc_nerfstudio, _ = nerfacc_nerfstudio.ray_aabb_intersect(
            origins, directions, aabb[None, :], near_plane=0, far_plane=1e10, miss_value=1e10
        )
        t_max_nerfacc_nerfstudio = t_max_nerfacc_nerfstudio.squeeze(-1)
        t_min_nerfacc_nerfstudio = t_min_nerfacc_nerfstudio.squeeze(-1)
        # time4 = time.time()

        # print("pytorch ", time2-time1)
        # print("nerfacc_nerfstudio ", time4-time3)

        assert torch.allclose(t_min, t_min_nerfacc_nerfstudio, rtol=0.001)
        assert torch.allclose(t_max, t_max_nerfacc_nerfstudio, rtol=0.001)
